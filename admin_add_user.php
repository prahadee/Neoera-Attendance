...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($username === '' || $password === '' || $full_name_f === '' || $employee_id === '' || $department === '' || $job_title === '' || $work_email === '') {
    ...
    if (empty($errors)) {
        $hashed = password_hash($password, PASSWORD_DEFAULT); // secure hash [web:28][web:34]
        ...
        try {
            ...
            $stmt2 = $conn->prepare("
                INSERT INTO employee_details 
                (user_id, full_name, employee_id, department, job_title, work_email) 
                VALUES (?, ?, ?, ?, ?, ?)
            ");
            $stmt2->bind_param("isssss", $new_user_id, $full_name_f, $employee_id, $department, $job_title, $work_email);
            $stmt2->execute();
            $stmt2->close();
...
// Avatar initials for logged-in admin
...
<head>
    ...
    <title>Add User | NeoEra Admin</title>
